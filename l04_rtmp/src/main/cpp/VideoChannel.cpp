...
//   数据 data      容器  pic_in
    ...
    memcpy(pic_in->img.plane[0], data, ySize);//y不变
    ...
    for (int i = 0; i < pi_nal; ++i) {
        //        单独发送的
        if (pp_nal[i].i_type == NAL_SPS) {//sps
            sps_len =   pp_nal[i].i_payload - 4;//-4为减去00000001的关键帧标志位
            memcpy(sps, pp_nal[i].p_payload + 4, sps_len);
        } else if (pp_nal[i].i_type == NAL_PPS) {//pps
            pps_len = pp_nal[i].i_payload - 4;
            memcpy(pps, pp_nal[i].p_payload + 4, pps_len);
            sendSpsPps(sps, pps, sps_len, pps_len);//单独发送sps pps 告诉解码器
        } else {
            // 非关键帧
...
// 非键帧/非关键帧发送：符合格式RTMP所需的格式packet
